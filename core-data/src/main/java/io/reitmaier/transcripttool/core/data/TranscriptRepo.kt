...
import io.reitmaier.transcripttool.core.data.mappers.*
...
class TranscriptRepo @Inject constructor(
  ...
  fun getPlayBackSpeed() : PlayBackSpeed =
      prefs.getEnum(prefsPlayBackSpeed, PlayBackSpeed.NORMAL)

  fun setPlayBackSpeed(playBackSpeed: PlayBackSpeed) {
    prefs.edit()
      .putEnum(prefsPlayBackSpeed,playBackSpeed)
      .apply()
  }

  fun getUserInfo() : DomainResult<UserInfo> = service.getUserInfo()

  suspend fun registerUser(name: Name, mobile: MobileNumber, operator: MobileOperator, password: Password = Password(UUID.randomUUID().toString())) : ApiResult<UserInfo> =
  ...
  suspend fun createProvisionalTask(incomingContent: IncomingContent) : DomainResult<ProvisionalTask> {
    val mimetypeExt = mimeTypeMap.getExtensionFromMimeType(incomingContent.mimeType) ?: "audio" // Default to generic audio extension
    val displayName = getDisplayName(incomingContent.uri).get()
    val fileInfo: File_Info = database.insertFileInfo(mimetypeExt,incomingContent.uri.toString(),displayName)
  ...
  suspend fun createTranscriptTask(provisionalTask: ProvisionalTask, durationMs: Long) : DomainResult<TranscriptTask> =
      service.submitTask(provisionalTask,durationMs)
        .andThen { database.insertLocalTask(it.remoteId, it.url, provisionalTask.fileId,provisionalTask.audio_path, provisionalTask.displayName, durationMs) }
...
//      val localFilePath = localEntity.path
      ...
      return@withContext if (regions.size != regionNumTarget) {
        ...
        for (n in 0 until regionNumTarget) {
          val region =
            database.insertRegion(id, ((n * regionLengthTarget)-750).coerceAtLeast(0), (n + 1) * regionLengthTarget - 1)
  ...
  suspend fun refreshTasks(): DomainResult<String> {
    return binding<String,DomainMessage> {
      ...
      remoteTasks.map { remoteTask ->
        ...
        if(localTask == null) {
          database.insertRemoteTask(remoteTask.id,"${TranscribeService.BASE_URL}/tasks/${remoteTask.id.value}/file", remoteTask.displayName, remoteTask.lengthMs ).bind()
          1
        } else {
          1
        }
